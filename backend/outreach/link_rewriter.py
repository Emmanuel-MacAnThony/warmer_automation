...
def make_signature_payload(eid: str, encoded_url: str) -> str:
    """Canonical payload the signature covers: eid|encoded_url."""
    return f"{eid}|{encoded_url}"


def verify_signature(eid: str, encoded_url: str, supplied_sig: str) -> bool:
    """Constant-time HMAC comparison for the /r endpoint."""
    expected = _sign(make_signature_payload(eid, encoded_url))
    return hmac.compare_digest(expected, supplied_sig or "")


def tracked_url(enrollment_id: Optional[int], original_url: str) -> str:
    """
    Build the signed redirect URL for a single original link.

    enrollment_id is optional — None means "this send isn't tied to a sequence
    enrollment" (e.g. a one-shot batch). The signature still authenticates the
    URL was generated by us so the /r endpoint refuses tampered links.
    """
    ...
    sig = _sign(make_signature_payload(eid, encoded))
    qs = urlencode({"eid": eid, "u": encoded, "s": sig})
...
def rewrite_links_in_body(html_body: str, enrollment_id: Optional[int]) -> str:
    """
    ...
    def _sub(m: re.Match[str]) -> str:
        ...
        return f"{prefix}{tracked_url(enrollment_id, href)}{suffix}"
...
def append_pitch_page_cta(
    ...
    tracked = tracked_url(enrollment_id, pitch_page_url)
...
def render_with_tracking(
    ...
    """
    body = rewrite_links_in_body(html_body or "", enrollment_id)
    body = append_pitch_page_cta(body, pitch_page_url, pitch_page_label, enrollment_id)
    return body
