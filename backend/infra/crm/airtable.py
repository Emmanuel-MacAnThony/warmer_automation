    ...
    def batch_update_in_table(
        ...
        for i in range(0, len(records), 10):
            ...
            try:
                ...
                err_str = str(e)
                # Unknown field — strip the offending field and retry the chunk
                # rather than silently dropping all 10 records
                if "UNKNOWN_FIELD_NAME" in err_str or "Unknown field name" in err_str:
                    import re as _re
                    bad = _re.search(r"[Uu]nknown field name[:\s]+['\"]?([^'\"]+)['\"]?", err_str)
                    bad_field = bad.group(1).strip() if bad else None
                    if bad_field:
                        logger.warning(
                            f"Chunk {i//10 + 1}: unknown field '{bad_field}' — retrying without it"
                        )
                        clean_chunk = [
                            {"id": r["id"], "fields": {k: v for k, v in r["fields"].items() if k != bad_field}}
                            for r in chunk
                        ]
                        try:
                            table.batch_update(clean_chunk, typecast=True)
                            updated += len(chunk)
                            continue
                        except Exception as e2:
                            logger.error(f"Chunk {i//10 + 1} retry failed: {e2}")
                    else:
                        logger.error(f"Chunk {i//10 + 1} unknown field error (could not parse field name): {e}")
                else:
                    logger.error(f"Error batch-updating chunk {i//10 + 1} in {base_id}/{table_id}: {e}")

        logger.info(f"Batch updated {updated}/{len(records)} records in {base_id}/{table_id}")
        return updated

    def ensure_table_fields(self, base_id: str, table_id: str, fields: List[Dict]) -> None:
        """
        Create fields in a table if they don't already exist.

        fields: list of {"name": str, "type": str, "options": dict (optional)}
        """
        ...
        schema = self.get_table_schema(base_id, table_id)
        ...
        for field in fields:
            ...
            if r.status_code == 200:
                logger.info("Created Airtable field '%s'", field["name"])
            else:
                logger.warning("Could not create field '%s': %s", field["name"], r.text)
